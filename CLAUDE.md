...
The sync requires two repository secrets: `DOCS_REPO_TOKEN` (a token with write access to the docs repo) and optionally `DOCS_REPO_NAME` (defaults to `ppl-ai/api-docs`). If the token is missing or cannot access the docs repo, the workflow logs a notice and skips the sync instead of failing.
